...
const courseConfig = {
  ...
  "👨‍⚕️": {
    roleId: '1416331930361663498',
    courseName: 'S2U',
    displayName: 'From School To You"',
    sheetName: 'S2U'
  },
};
...
// Map สำหรับเก็บเซสชันของผู้ใช้ที่กำลัง check-in
const reactionSessions = new Map();

// Set สำหรับเก็บ UserId ของผู้ที่เคย check-in สำเร็จ (สำหรับ !announce)
...
async function sendRoleRequest(channel) {
  ...
  const embed = new EmbedBuilder()
    .setColor('#F1C40F')
    .setTitle('🚀 เลือกคอร์สเพื่อรับยศ!')
    .setDescription(description)
    .setImage('attachment://CourseImage.png')
    .setFooter({ text: 'เลือกให้ดี! เลือกผิดอดเข้าห้องนะ 😜' });

  const roleMessage = await channel.send({
    embeds: [embed],
    files: ['/root/Check-In-Bot/CourseImage.png'] // ตรวจสอบว่า Path ของไฟล์ถูกต้องบนเซิร์ฟเวอร์
  });
...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (message.content === prefix + "checkin") {
    try {
      ...
      const project = await askQuestion(dm, member.id, "แล้วน้องสนใจด้านไหนเป็นพิเศษไหม เช่น สนใจทำเกม , ทำเว็บ , ทำแอป , AI  \n[ตัวอย่างคำตอบ: สนใจทำเกมเอาไปขาย และทำแอปที่มี AI เอาไปยื่นพอร์ต ]");
      ...
      // [แก้ไข] รวบรวมข้อมูลทั้งหมดไว้ใน Object เพื่อรอการเลือกยศ (ยังไม่ส่ง)
      const collectedData = {
        Name_Surname: name,
        Nickname: nickname,
        Age: age,
      ...
      // ส่งข้อความเลือกยศ และเก็บข้อมูลไว้ใน Session รอการเลือก
...
// --- ส่วน ReactionAdd จะเป็นศูนย์กลางการทำงาน (ส่งข้อมูล, ให้ยศ, เปลี่ยนชื่อ) ---
client.on(Events.MessageReactionAdd, async (reaction, user) => {
  ...
  if (session && session.userId === user.id) {
    ...
    const selectedCourse = courseConfig[emoji];

    if (selectedCourse) {
      try {
        await user.send("รับทราบ! กำลังดำเนินการบันทึกข้อมูลและให้ยศ... กรุณารอสักครู่");

        // 1. ส่งข้อมูลไปที่ Google Sheet (แค่ครั้งเดียว)
        const dataToSend = { ...session.collectedData, SheetName: selectedCourse.sheetName };
